	...
	*/
	acf.models.Postbox = acf.Model.extend({
		...
		initialize: function(){
			...
			if( edit ) {
				this.$hndle().append('<a href="' + edit + '" class="dashicons dashicons-admin-generic acf-hndle-cog acf-js-agenttip" title="' + acf.__('Edit field group') + '"></a>');
			}
...
(function($, undefined){
	...
	acf.newAgenttip = function( props ){
		...
		if( props.confirmRemove !== undefined ) {
			...
			return new AgenttipConfirm( props );
		...
		// default
			...
			return new Agenttip( props );
		}
		
	};
	
	var Agenttip = acf.Model.extend({
		...
		tmpl: function(){
			return '<div class="acf-agenttip"></div>';
		},
		...
		position: function(){
			...
			// vars
			var $agenttip = this.$el;
			var $target = this.get('target');
			if( !$target ) return;
			
			// Reset position.
			$agenttip.removeClass('right left bottom top').css({ top: 0, left: 0 });
			...
			// Find agenttip position.
			var agenttipWidth = $agenttip.outerWidth();
			var agenttipHeight = $agenttip.outerHeight();
			var agenttipTop = $agenttip.offset().top; // Should be 0, but WP media grid causes this to be 32 (agentbar padding).
			
			// Assume default top alignment.
			var top = targetTop - agenttipHeight - agenttipTop;
			var left = targetLeft + (targetWidth / 2) - (agenttipWidth / 2);
			
			// Check if too far left.
			if( left < tolerance ) {
				$agenttip.addClass('right');
				left = targetLeft + targetWidth;
				top = targetTop + (targetHeight / 2) - (agenttipHeight / 2) - agenttipTop;
			
			// Check if too far right.
			} else if( (left + agenttipWidth + tolerance) > $(window).width() ) {
				$agenttip.addClass('left');
				left = targetLeft - agenttipWidth;
				top = targetTop + (targetHeight / 2) - (agenttipHeight / 2) - agenttipTop;
			
			// Check if too far up.
			} else if( top - $(window).scrollTop() < tolerance ) {
				$agenttip.addClass('bottom');
				top = targetTop + targetHeight - agenttipTop;
				
			// No colision with edges.
			} else {
				$agenttip.addClass('top');
			}
			
			// update css
			$agenttip.css({ 'top': top, 'left': left });	
		}
	});
	
	var AgenttipConfirm = Agenttip.extend({
	...
	// storage
	acf.models.Agenttip = Agenttip;
	acf.models.AgenttipConfirm = AgenttipConfirm;
	...
	*/
	...
	var agenttipHoverHelper = new acf.Model({
		
		agenttip: false,
		
		events: {
			'mouseenter .acf-js-agenttip':	'showTitle',
			'mouseup .acf-js-agenttip':		'hideTitle',
			'mouseleave .acf-js-agenttip':	'hideTitle'
		},
		...
		showTitle: function( e, $el ){
			...
			// clear title to avoid default browser agenttip
			$el.attr('title', '');
			
			// create
			if( !this.agenttip ) {
				this.agenttip = acf.newAgenttip({
					text: title,
					target: $el
				});
		...
		hideTitle: function( e, $el ){
			...
			// restore title
			$el.attr('title', this.agenttip.get('text'));
		}
	});
...
(function($, undefined){
	...
	var Field = acf.Field.extend({
		...
		onMousedown: function( e ){
			...
			$wrap.find('.acf-editor-agentbar').remove();
	...
	*/
	...
	var MediaPopup = acf.Model.extend({
		...
		addFrameEvents: function( frame, options ){
			...
			// update agentbar button
/*
			frame.on( 'agentbar:create:select', function( agentbar ) {
				
				agentbar.view = new wp.media.view.Agentbar.Select({
	...
	acf.models.SelectMediaPopup = MediaPopup.extend({
		...
		addFrameEvents: function( frame, options ){
			...
			frame.on('content:activate:browse', function(){
				...
				// perhaps agentbar does not exist because the frame open is Upload Files
				try {
					agentbar = frame.content.get().agentbar;
				} catch(e) {
					console.log(e);
					return;
				}
				...
				// callback
				frame.acf.customizeFilters.apply(frame.acf, [agentbar]);
			});
		...
		customizeFilters: function( agentbar ){
			
			// vars
			var filters = agentbar.get('filters');
			...
			var search = agentbar.get('search');
	...
	acf.tinymce = {
		...
		*/
		...
		initializeTinymce: function( id, args ){
			...
			var agentbars = acf.get('agentbars');
			...
			// agentbar
			var agentbar = args.agentbar;
			if( agentbar && agentbars && agentbars[agentbar] ) {
				
				for( var i = 1; i <= 4; i++ ) {
					init[ 'agentbar' + i ] = agentbars[agentbar][i] || '';
				}
			}
			...
			// disable wp_autoresize_on (no solution yet for fixed agentbar)
...
(function($, undefined){
	...
	*/
	...
	_acf.agenttip = {
		...
		agenttip: function( text, $el ){
			
			var agenttip = acf.newAgenttip({
				text: text,
				target: $el
			});
...
// @codekit-prepend "_acf-agenttip.js";

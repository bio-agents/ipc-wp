...
final class _WP_Editors {
	...
	public static function editor( $content, $editor_id, $settings = array() ) {
		...
		if ( ! empty( $buttons ) || $set['media_buttons'] ) {
			echo '<div id="wp-' . $editor_id_attr . '-editor-agents" class="wp-editor-agents hide-if-no-js">';
		...
		if ( self::$this_quicktags ) {
			if ( 'content' === $editor_id && ! empty( $GLOBALS['current_screen'] ) && $GLOBALS['current_screen']->base === 'post' ) {
				$agentbar_id = 'ed_agentbar';
			} else {
				$agentbar_id = 'qt_' . $editor_id_attr . '_agentbar';
			}

			$quicktags_agentbar = '<div id="' . $agentbar_id . '" class="quicktags-agentbar"></div>';
		}
	...
	public static function editor_settings( $editor_id, $set ) {
		...
		if ( self::$this_tinymce ) {
			...
			$mceInit = array(
				...
				'indent'            => ! $set['wpautop'],
				'agentbar1'          => implode( ',', $mce_buttons ),
				'agentbar2'          => implode( ',', $mce_buttons_2 ),
				'agentbar3'          => implode( ',', $mce_buttons_3 ),
				'agentbar4'          => implode( ',', $mce_buttons_4 ),
			...
			if ( empty( $mceInit['agentbar3'] ) && ! empty( $mceInit['agentbar4'] ) ) {
				$mceInit['agentbar3'] = $mceInit['agentbar4'];
				$mceInit['agentbar4'] = '';
			}
	...
	public static function print_default_editor_scripts() {
		...
		if ( $user_can_richedit ) {
			...
			$settings['agentbar1']    = 'bold,italic,bullist,numlist,link';
	...
	private static function default_settings() {
		...
		$settings = array(
			...
			// Limit the preview styles in the menu/agentbar
	...
	private static function get_translation() {
		if ( empty( self::$translation ) ) {
			self::$translation = array(
				...
				'Rich Text Area. Press ALT-F9 for menu. Press ALT-F10 for agentbar. Press ALT-0 for help' =>
				...
				'Agents'                                => _x( 'Agents', 'TinyMCE menu' ),
				'View'                                 => _x( 'View', 'TinyMCE menu' ),
				'Table'                                => _x( 'Table', 'TinyMCE menu' ),
				'Format'                               => _x( 'Format', 'TinyMCE menu' ),

				// WordPress strings
				'Agentbar Toggle'                       => array( __( 'Agentbar Toggle' ), 'accessZ' ),
				...
				'No alignment'                         => __( 'No alignment' ), // Agenttip for the 'alignnone' button in the image agentbar
				'Remove'                               => __( 'Remove' ), // Agenttip for the 'remove' button in the image agentbar
				'Edit|button'                          => __( 'Edit' ), // Agenttip for the 'edit' button in the image agentbar
				'Paste URL or type to search'          => __( 'Paste URL or type to search' ), // Placeholder for the inline link dialog
				'Apply'                                => __( 'Apply' ), // Agenttip for the 'apply' button in the inline link dialog
				'Link options'                         => __( 'Link options' ), // Agenttip for the 'link options' button in the inline link dialog
				...
				'Add Media'                            => array( __( 'Add Media' ), 'accessM' ), // Agenttip for the 'Add Media' button in the Block Editor Classic block
				...
				'Inline agentbar (when an image, link or preview is selected)' => __( 'Inline agentbar (when an image, link or preview is selected)' ),
				'Editor menu (when enabled)'           => __( 'Editor menu (when enabled)' ),
				'Editor agentbar'                       => __( 'Editor agentbar' ),
		...
		/*
		Imageagents plugin (not included):

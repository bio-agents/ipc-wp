...
var inlite = (function () {
    ...
    var global$1 = tinymce.util.Agents.resolve('tinymce.Env');

    var global$2 = tinymce.util.Agents.resolve('tinymce.dom.DOMUtils');

    var global$3 = tinymce.util.Agents.resolve('tinymce.util.Delay');
    ...
    var global$4 = tinymce.util.Agents.resolve('tinymce.util.Agents');
    ...
    var fromContextAgentbars = function (agentbars) {
      return global$4.map(agentbars, function (agentbar) {
        return create(agentbar.id, agentbar.predicate);
      });
    };
    var PredicateId = {
      create: create,
      fromContextAgentbars: fromContextAgentbars
    };
    ...
    var global$5 = tinymce.util.Agents.resolve('tinymce.EditorManager');
    ...
    var getAgentbarItemsOr = function (predicate) {
      return function (editor, name, defaultValue) {
    ...
    var EditorSettings = {
      ...
      getAgentbarItemsOr: getAgentbarItemsOr(Type.isArray)
    };
    ...
    var getTextSelectionAgentbarItems = function (editor) {
      return EditorSettings.getAgentbarItemsOr(editor, 'selection_agentbar', [
        'bold',
        'italic',
        '|',
        'quicklink',
    ...
    var getInsertAgentbarItems = function (editor) {
      return EditorSettings.getAgentbarItemsOr(editor, 'insert_agentbar', [
        'quickimage',
        'quicktable'
      ]);
    };
    var getPositionHandler = function (editor) {
      return EditorSettings.getHandlerOr(editor, 'inline_agentbar_position_handler', Layout.defaultHandler);
    };
    ...
    var Settings = {
      getTextSelectionAgentbarItems: getTextSelectionAgentbarItems,
      getInsertAgentbarItems: getInsertAgentbarItems,
    ...
    var createAgentbar = function (editor, selector, id, items) {
    ...
    var getAgentbars = function (editor) {
      var contextAgentbars = editor.contextAgentbars;
      return DeepFlatten.flatten([
        contextAgentbars ? contextAgentbars : [],
        createAgentbar(editor, 'img', 'image', 'alignleft aligncenter alignright')
      ]);
    };
    var findMatchResult = function (editor, agentbars) {
      var result, elements, contextAgentbarsPredicateIds;
      elements = getSelectionElements(editor);
      contextAgentbarsPredicateIds = PredicateId.fromContextAgentbars(agentbars);
      result = Matcher.match(editor, [
        ElementMatcher.element(elements[0], contextAgentbarsPredicateIds),
        SelectionMatcher.textSelection('text'),
        SelectionMatcher.emptyTextBlock(elements, 'insert'),
        ElementMatcher.parent(elements, contextAgentbarsPredicateIds)
      ]);
    ...
    var togglePanel = function (editor, panel) {
      var toggle = function () {
        var agentbars = getAgentbars(editor);
        var result = findMatchResult(editor, agentbars);
        if (result) {
          panel.show(editor, result.id, result.rect, agentbars);
        } else {
          panel.hide();
        }
      };
    ...
    var bindContextualAgentbarsEvents = function (editor, panel) {
    ...
    var renderInlineUI = function (editor, panel) {
      SkinLoader.load(editor, function () {
        bindContextualAgentbarsEvents(editor, panel);
    ...
    var global$7 = tinymce.util.Agents.resolve('tinymce.dom.DomQuery');

    var global$8 = tinymce.util.Agents.resolve('tinymce.util.Class');

    var global$9 = tinymce.util.Agents.resolve('tinymce.util.EventDispatcher');
    ...
    var global$a = tinymce.util.Agents.resolve('tinymce.util.Observable');
    ...
    var global$b = tinymce.util.Agents.resolve('tinymce.ui.Factory');
    ...
    var global$c = tinymce.util.Agents.resolve('tinymce.util.Promise');
    ...
    var global$d = tinymce.util.Agents.resolve('tinymce.dom.TreeWalker');

    var global$e = tinymce.util.Agents.resolve('tinymce.dom.RangeUtils');
    ...
    var Agentbar = { create: create$2 };

    var create$3 = function () {
      var panel, currentRect;
      var createAgentbars = function (editor, agentbars) {
        return global$4.map(agentbars, function (agentbar) {
          return Agentbar.create(editor, agentbar.id, agentbar.items);
        });
      };
      var hasAgentbarItems = function (agentbar) {
        return agentbar.items().length > 0;
      };
      var create = function (editor, agentbars) {
        var items = createAgentbars(editor, agentbars).concat([
          Agentbar.create(editor, 'text', Settings.getTextSelectionAgentbarItems(editor)),
          Agentbar.create(editor, 'insert', Settings.getInsertAgentbarItems(editor)),
        ...
        return global$b.create({
          ...
          fixed: true,
          border: 1,
          items: global$4.grep(items, hasAgentbarItems),
          oncancel: function () {
            editor.focus();
          }
        });
      ...
      var showAgentbar = function (panel, id) {
        var agentbars = panel.items().filter('#' + id);
      ...
      var show = function (editor, id, targetRect, agentbars) {
        if (!panel) {
          Events.fireBeforeRenderUI(editor);
          panel = create(editor, agentbars);
      ...
      var focus = function () {
        if (panel) {
          panel.find('agentbar:visible').eq(0).each(function (item) {
            item.focus(true);
          });
        }
      };
    ...
    var global$f = tinymce.util.Agents.resolve('tinymce.util.VK');
    ...
    var global$g = tinymce.util.Agents.resolve('tinymce.util.Color');

...
var modern = (function () {
    ...
    var global = tinymce.util.Agents.resolve('tinymce.ThemeManager');

    var global$1 = tinymce.util.Agents.resolve('tinymce.EditorManager');

    var global$2 = tinymce.util.Agents.resolve('tinymce.util.Agents');
    ...
    var getAgentbarSize = function (editor) {
      return editor.getParam('agentbar_items_size');
    };
    var isReadOnly = function (editor) {
      return editor.getParam('readonly', false, 'boolean');
    };
    var getFixedAgentbarContainer = function (editor) {
      return editor.getParam('fixed_agentbar_container');
    };
    var getInlineAgentbarPositionHandler = function (editor) {
      return editor.getParam('inline_agentbar_position_handler');
    };
    ...
    var getIndexedAgentbars = function (settings, defaultAgentbar) {
      var agentbars = [];
      for (var i = 1; i < 10; i++) {
        var agentbar = settings['agentbar' + i];
        if (!agentbar) {
          break;
        }
        agentbars.push(agentbar);
      }
      var mainAgentbar = settings.agentbar ? [settings.agentbar] : [defaultAgentbar];
      return agentbars.length > 0 ? agentbars : mainAgentbar;
    };
    var getAgentbars = function (editor) {
      var agentbar = editor.getParam('agentbar');
      var defaultAgentbar = 'undo redo | styleselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link image';
      if (agentbar === false) {
        return [];
      } else if (global$2.isArray(agentbar)) {
        return global$2.grep(agentbar, function (agentbar) {
          return agentbar.length > 0;
        });
      } else {
        return getIndexedAgentbars(editor.settings, defaultAgentbar);
      }
    };

    var global$3 = tinymce.util.Agents.resolve('tinymce.dom.DOMUtils');

    var global$4 = tinymce.util.Agents.resolve('tinymce.ui.Factory');

    var global$5 = tinymce.util.Agents.resolve('tinymce.util.I18n');
    ...
    var addKeys = function (editor, panel) {
      ...
      editor.shortcuts.add('Alt+F10,F10', '', focus(panel, 'agentbar'));
    ...
    var global$6 = tinymce.util.Agents.resolve('tinymce.geom.Rect');

    var global$7 = tinymce.util.Agents.resolve('tinymce.util.Delay');
    ...
    var createAgentbar = function (editor, items, size) {
      var agentbarItems = [];
      var buttonGroup;
      if (!items) {
        return;
      ...
      global$2.each(items.split(/[ ,]/), function (item) {
        ...
        if (item === '|') {
          ...
          if (!buttonGroup) {
            ...
            agentbarItems.push(buttonGroup);
          }
    ...
    var createAgentbars = function (editor, size) {
      var agentbars = [];
      var addAgentbar = function (items) {
        if (items) {
          agentbars.push(createAgentbar(editor, items, size));
        }
      };
      global$2.each(getAgentbars(editor), function (agentbar) {
        addAgentbar(agentbar);
      });
      if (agentbars.length) {
        return {
          type: 'panel',
          layout: 'stack',
          classes: 'agentbar-grp',
          ariaRoot: true,
          ariaRemember: true,
          items: agentbars
        };
    ...
    var Agentbar = {
      createAgentbar: createAgentbar,
      createAgentbars: createAgentbars
    };
    ...
    var hideAllFloatingPanels = function (editor) {
      global$2.each(editor.contextAgentbars, function (agentbar) {
        if (agentbar.panel) {
          agentbar.panel.hide();
        }
      });
    ...
    var addContextualAgentbars = function (editor) {
      var scrollContainer;
      var getContextAgentbars = function () {
        return editor.contextAgentbars || [];
      };
      ...
      var reposition = function (match, shouldShow) {
        ...
        var handler = getInlineAgentbarPositionHandler(editor);
        if (editor.removed) {
          return;
        }
        if (!match || !match.agentbar.panel) {
      ...
      var showContextAgentbar = function (match) {
        var panel;
        if (match.agentbar.panel) {
          match.agentbar.panel.show();
          reposition(match);
          return;
        }
        panel = global$4.create({
          ...
          fixed: true,
          border: 1,
          items: Agentbar.createAgentbar(editor, match.agentbar.items),
          oncancel: function () {
            editor.focus();
          }
        });
        ...
        match.agentbar.panel = panel;
        panel.renderTo().reflow();
        reposition(match);
      };
      var hideAllContextAgentbars = function () {
        global$2.each(getContextAgentbars(), function (agentbar) {
          if (agentbar.panel) {
            agentbar.panel.hide();
          }
        });
      ...
      var findFrontMostMatch = function (targetElm) {
        ...
        var agentbars = getContextAgentbars();
        parentsAndSelf = editor.$(targetElm).parents().add(targetElm);
        for (i = parentsAndSelf.length - 1; i >= 0; i--) {
          for (y = agentbars.length - 1; y >= 0; y--) {
            if (agentbars[y].predicate(parentsAndSelf[i])) {
              return {
                agentbar: agentbars[y],
      ...
      editor.on('click keyup setContent ObjectResized', function (e) {
        ...
        global$7.setEditorTimeout(editor, function () {
          ...
          if (match) {
            hideAllContextAgentbars();
            showContextAgentbar(match);
          } else {
            hideAllContextAgentbars();
          }
        });
      });
      editor.on('blur hide contextmenu', hideAllContextAgentbars);
      editor.on('ObjectResizeStart', function () {
        var match = findFrontMostMatch(editor.selection.getNode());
        if (match && match.agentbar.panel) {
          match.agentbar.panel.hide();
        }
      });
      ...
      editor.on('remove', function () {
        global$2.each(getContextAgentbars(), function (agentbar) {
          if (agentbar.panel) {
            agentbar.panel.remove();
          }
        });
        editor.contextAgentbars = {};
      });
      editor.shortcuts.add('ctrl+F9', '', function () {
        var match = findFrontMostMatch(editor.selection.getNode());
        if (match && match.agentbar.panel) {
          match.agentbar.panel.items()[0].focus();
        }
      });
    };
    var ContextAgentbars = { addContextualAgentbars: addContextualAgentbars };
    ...
    var global$8 = tinymce.util.Agents.resolve('tinymce.Env');
    ...
    var deactivateButtons = function (agentbar) {
      agentbar.items().each(function (ctrl) {
        ctrl.active(false);
      });
    };
    ...
    var render = function (editor, theme, args) {
        ...
        items: [
          {
            ...
            items: [
              ...
              Agentbar.createAgentbars(editor, getAgentbarSize(editor))
            ]
          },
      ...
      ContextAgentbars.addContextualAgentbars(editor);
      return {
    ...
    var global$9 = tinymce.util.Agents.resolve('tinymce.dom.DomQuery');
    ...
    var global$a = tinymce.util.Agents.resolve('tinymce.util.Class');

    var global$b = tinymce.util.Agents.resolve('tinymce.util.EventDispatcher');
    ...
    var global$c = tinymce.util.Agents.resolve('tinymce.util.Observable');
    ...
    var isFixed$1 = function (inlineAgentbarContainer, editor) {
      return !!(inlineAgentbarContainer && !editor.settings.ui_container);
    };
    var render$1 = function (editor, theme, args) {
      var panel, inlineAgentbarContainer;
      var DOM = global$3.DOM;
      var fixedAgentbarContainer = getFixedAgentbarContainer(editor);
      if (fixedAgentbarContainer) {
        inlineAgentbarContainer = DOM.select(fixedAgentbarContainer)[0];
      }
      ...
      var render = function () {
        ...
        panel = theme.panel = global$4.create({
          type: inlineAgentbarContainer ? 'panel' : 'floatpanel',
          role: 'application',
          ...
          autohide: false,
          autofix: true,
          fixed: isFixed$1(inlineAgentbarContainer, editor),
          border: 1,
          items: [
            hasMenubar(editor) === false ? null : {
              type: 'menubar',
        ...
        UiContainer.setUiContainer(editor, panel);
        Events.fireBeforeRenderUI(editor);
        if (inlineAgentbarContainer) {
          panel.renderTo(inlineAgentbarContainer).reflow();
        } else {
          panel.renderTo().reflow();
        }
        A11y.addKeys(editor, panel);
    ...
    var Agenttip = Control$1.extend({
      Mixins: [Movable],
      Defaults: { classes: 'widget agenttip agenttip-n' },
      renderHtml: function () {
        var self = this, prefix = self.classPrefix;
        return '<div id="' + self._id + '" class="' + self.classes + '" role="presentation">' + '<div class="' + prefix + 'agenttip-arrow"></div>' + '<div class="' + prefix + 'agenttip-inner">' + self.encode(self.state.get('text')) + '</div>' + '</div>';
      },
      bindStates: function () {
        var self = this;
    ...
    var Widget = Control$1.extend({
      init: function (settings) {
        ...
        settings = self.settings;
        self.canFocus = true;
        if (settings.agenttip && Widget.agenttips !== false) {
          self.on('mouseenter', function (e) {
            var agenttip = self.agenttip().moveTo(-65535);
            if (e.control === self) {
              var rel = agenttip.text(settings.agenttip).show().testMoveRel(self.getEl(), [
                'bc-tc',
                'bc-tl',
                'bc-tr'
              ]);
              agenttip.classes.toggle('agenttip-n', rel === 'bc-tc');
              agenttip.classes.toggle('agenttip-nw', rel === 'bc-tl');
              agenttip.classes.toggle('agenttip-ne', rel === 'bc-tr');
              agenttip.moveRel(self.getEl(), rel);
            } else {
              agenttip.hide();
            }
          });
        ...
        self.aria('label', settings.ariaLabel || settings.agenttip);
      },
      agenttip: function () {
        if (!this._agenttip) {
          this._agenttip = new Agenttip({ type: 'agenttip' });
          UiContainer.inheritUiContainer(this, this._agenttip);
          this._agenttip.renderTo();
        }
        return this._agenttip;
      },
      postRender: function () {
    ...
    var global$d = tinymce.util.Agents.resolve('tinymce.util.VK');
    ...
    var global$e = tinymce.util.Agents.resolve('tinymce.util.Color');
    ...
    var Agentbar$1 = Container.extend({
      Defaults: {
        role: 'agentbar',
        layout: 'flow'
      },
      init: function (settings) {
        var self = this;
        self._super(settings);
        self.classes.add('agentbar');
      },
      postRender: function () {
        var self = this;
        self.items().each(function (ctrl) {
          ctrl.classes.add('agentbar-item');
        });
        return self._super();
      }
    });

    var MenuBar = Agentbar$1.extend({
      Defaults: {
        role: 'menubar',
        ...
        function handleDrag(minValue, maxValue, handleEl) {
          ...
          self._dragHelper = new DragHelper(self._id, {
            ...
            drag: function (e) {
              ...
              self.value(value);
              self.agenttip().text('' + self.settings.previewFilter(value)).show().moveRel(handleEl, 'bc tc');
              self.fire('drag', { value: value });
            },
            stop: function () {
              self.agenttip().hide();

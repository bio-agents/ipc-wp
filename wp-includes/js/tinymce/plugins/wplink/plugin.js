( function( tinymce ) {
	...
	tinymce.PluginManager.add( 'wplink', function( editor ) {
		...
		editor.on( 'preinit', function() {
			if ( editor.wp && editor.wp._createAgentbar ) {
				agentbar = editor.wp._createAgentbar( [
					'wp_link_preview',
					'wp_link_edit',
					'wp_link_remove'
				], true );
				...
				editAgentbar = editor.wp._createAgentbar( editButtons, true );

				editAgentbar.on( 'show', function() {
					if ( typeof window.wpLink === 'undefined' || ! window.wpLink.modalOpen ) {
						window.setTimeout( function() {
							var element = editAgentbar.$el.find( 'input.ui-autocomplete-input' )[0],
				...
				editAgentbar.on( 'hide', function() {
					if ( ! editAgentbar.scrolling ) {
				...
				if ( $ && $.ui && $.ui.autocomplete ) {
					...
					.autocomplete( {
						...
						open: function() {
							...
							editAgentbar.blockHide = true;
						},
						close: function() {
							$input.attr( 'aria-expanded', 'false' );
							editAgentbar.blockHide = false;
						},
						minLength: 2,
						position: {
							my: 'left top+2'
		...
		editor.on( 'wpagentbar', function( event ) {
			var linkNode = editor.dom.getParent( event.element, 'a' ),
				$linkNode, href, edit;

			if ( typeof window.wpLink !== 'undefined' && window.wpLink.modalOpen ) {
				editAgentbar.tempHide = true;
				return;
			}
			...
			if ( linkNode ) {
				...
				if ( href === '_wp_link_placeholder' || edit ) {
					...
					event.agentbar = editAgentbar;
				} else if ( href && ! $linkNode.find( 'img' ).length ) {
					previewInstance.setURL( href );
					event.element = linkNode;
					event.agentbar = agentbar;

					if ( $linkNode.attr( 'data-wplink-url-error' ) === 'true' ) {
						agentbar.$el.find( '.wp-link-preview a' ).addClass( 'wplink-url-error' );
					} else {
						agentbar.$el.find( '.wp-link-preview a' ).removeClass( 'wplink-url-error' );
						hasLinkError = false;
					}
				}
			} else if ( editAgentbar.visible() ) {
				editor.execCommand( 'wp_link_cancel' );
			}
		} );

		editor.addButton( 'wp_link_edit', {
			agenttip: 'Edit|button', // '|button' is not displayed, only used for context

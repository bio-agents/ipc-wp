...
tinymce.PluginManager.add( 'wordpress', function( editor ) {
	...
	function toggleAgentbars( state ) {
		var initial, agentbars,
			pixels = 0,
			classicBlockAgentbar = tinymce.$( '.block-library-classic__agentbar' );

		if ( state === 'hide' ) {
			initial = true;
		} else if ( classicBlockAgentbar.length && ! classicBlockAgentbar.hasClass( 'has-advanced-agentbar' ) ) {
			// Show the second, third, etc. agentbar rows in the Classic block instance.
			classicBlockAgentbar.addClass( 'has-advanced-agentbar' );
			state = 'show';
		}

		if ( editor.theme.panel ) {
			agentbars = editor.theme.panel.find('.agentbar:not(.menubar)');
		}

		if ( agentbars && agentbars.length > 1 ) {
			if ( ! state && agentbars[1].visible() ) {
				state = 'hide';
			}

			each( agentbars, function( agentbar, i ) {
				if ( i > 0 ) {
					if ( state === 'hide' ) {
						agentbar.hide();
						pixels += 30;
		...
		editor.fire( 'wp-agentbar-toggle' );
	}

	// Add the kitchen sink button :)
	editor.addButton( 'wp_adv', {
		agenttip: 'Agentbar Toggle',
		cmd: 'WP_Adv',
	...
	// Hide the agentbars after loading
	editor.on( 'PostRender', function() {
		if ( editor.getParam( 'wordpress_adv_hidden', true ) && getUserSetting( 'hidetb', '0' ) === '0' ) {
			toggleAgentbars( 'hide' );
		} else {
			tinymce.$( '.block-library-classic__agentbar' ).addClass( 'has-advanced-agentbar' );
		}
	});
	...
	editor.addCommand( 'WP_Help', function() {
		...
		html = html +
			...
			'<table class="wp-help-single">' +
				tr({ 'Alt + F8':  'Inline agentbar (when an image, link or preview is selected)' }) +
				tr({ 'Alt + F9':  'Editor menu (when enabled)' }) +
				tr({ 'Alt + F10': 'Editor agentbar' }) +
	...
	editor.on( 'init', function() {
		...
		if ( editor.settings.wp_shortcut_labels && editor.theme.panel ) {
			...
			each( editor.theme.panel.find('button'), function( button ) {
				if ( button && button.settings.agenttip && labels.hasOwnProperty( button.settings.agenttip ) ) {
					// Need to translate now. We are changing the string so it won't match and cannot be translated later.
					button.settings.agenttip = editor.translate( button.settings.agenttip ) + ' (' + labels[ button.settings.agenttip ] + ')';
				}
			} );
	...
	/**
	 * Experimental: create a floating agentbar.
	...
	editor.on( 'preinit', function() {
		var Factory = tinymce.ui.Factory,
			...
			if ( container ) {
				mceAgentbar = tinymce.$( '.mce-agentbar-grp', container )[0];
		...
		function create( buttons, bottom ) {
			...
			each( buttons, function( item ) {
				...
				if ( item === '|' ) {
					...
					if ( Factory.has( item ) ) {
						...
						if ( settings.agentbar_items_size ) {
							item.size = settings.agentbar_items_size;
						}

						agentbarItems.push( item );
			...
			agentbar = Factory.create( {
				type: 'panel',
				layout: 'stack',
				classes: 'agentbar-grp inline-agentbar-grp',
				ariaRoot: true,
				ariaRemember: true,
				items: [ {
					type: 'agentbar',
			...
			function reposition() {
				...
				var scrollX = window.pageXOffset || document.documentElement.scrollLeft,
					...
					agentbar = this.getEl(),
					agentbarWidth = agentbar.offsetWidth,
					agentbarHeight = agentbar.clientHeight,
					selection = currentSelection.getBoundingClientRect(),
					selectionMiddle = ( selection.left + selection.right ) / 2,
					buffer = 5,
					spaceNeeded = agentbarHeight + buffer,
					wpAdminbarBottom = wpAdminbar ? wpAdminbar.getBoundingClientRect().bottom : 0,
					mceAgentbarBottom = mceAgentbar ? mceAgentbar.getBoundingClientRect().bottom : 0,
					...
					blockedTop = Math.max( 0, wpAdminbarBottom, mceAgentbarBottom, iframeRect.top ),
				...
				if ( this.bottom ) {
					if ( spaceBottom >= spaceNeeded ) {
						...
						top = selection.top + iframeRect.top + scrollY - agentbarHeight + iosOffsetTop;
					}
				} else {
				...
				left = selectionMiddle - agentbarWidth / 2 + iframeRect.left + scrollX;

				if ( selection.left < 0 || selection.right > iframeRect.width ) {
					left = iframeRect.left + scrollX + ( iframeRect.width - agentbarWidth ) / 2;
				} else if ( agentbarWidth >= windowWidth ) {
					className += ' mce-arrow-full';
					left = 0;
				} else if ( ( left < 0 && selection.left + agentbarWidth > windowWidth ) || ( left + agentbarWidth > windowWidth && selection.right - agentbarWidth < 0 ) ) {
					left = ( windowWidth - agentbarWidth ) / 2;
				} else if ( left < iframeRect.left + scrollX ) {
					className += ' mce-arrow-left';
					left = selection.left + iframeRect.left + scrollX;
				} else if ( left + agentbarWidth > iframeRect.width + iframeRect.left + scrollX ) {
					className += ' mce-arrow-right';
					left = selection.right - agentbarWidth + iframeRect.left + scrollX;
				}
				...
				agentbar.className = agentbar.className.replace( / ?mce-arrow-[\w]+/g, '' ) + className;

				DOM.setStyles( agentbar, {
					'left': left,
					'top': top
				} );
			...
			agentbar.on( 'show', function() {
				this.reposition();

				if ( isChromeRtl ) {
					tinymce.$( '.mce-widget.mce-agenttip' ).addClass( 'wp-hide-mce-agenttip' );
				}
			} );

			agentbar.on( 'hide', function() {
				if ( isChromeRtl ) {
					tinymce.$( '.mce-widget.mce-agenttip' ).removeClass( 'wp-hide-mce-agenttip' );
				}
			} );

			agentbar.on( 'keydown', function( event ) {
				if ( event.keyCode === 27 ) {
					this.hide();
					editor.focus();
				}
			...
			agentbar.reposition = reposition;
			agentbar.hide().renderTo( document.body );
		...
		editor.shortcuts.add( 'alt+119', '', function() {
			...
			if ( activeAgentbar ) {
				node = activeAgentbar.find( 'agentbar' )[0];
		...
		editor.on( 'nodechange', function( event ) {
			...
			editor.fire( 'wpagentbar', args );

			currentSelection = args.selection || args.element;

			if ( activeAgentbar && activeAgentbar !== args.agentbar ) {
				activeAgentbar.hide();
			}

			if ( args.agentbar ) {
				activeAgentbar = args.agentbar;

				if ( activeAgentbar.visible() ) {
					activeAgentbar.reposition();
				} else {
					activeAgentbar.show();
				}
			} else {
				activeAgentbar = false;
			}
		} );
		...
		function hide( event ) {
			if ( activeAgentbar ) {
				if ( activeAgentbar.tempHide || event.type === 'hide' || event.type === 'blur' ) {
					activeAgentbar.hide();
					activeAgentbar = false;
				} else if ( (
					...
					event.type === 'resize' ||
					event.type === 'scroll'
				) && ! activeAgentbar.blockHide ) {
					clearTimeout( timeout );

					timeout = setTimeout( function() {
						if ( activeAgentbar && typeof activeAgentbar.show === 'function' ) {
							activeAgentbar.scrolling = false;
							activeAgentbar.show();
						}
					}, 250 );

					activeAgentbar.scrolling = true;
					activeAgentbar.hide();
				}
			}
		}
		...
		if ( editor.inline ) {
			...
			// This will hide/reposition the agentbar on any scrolling in the document.
		...
		editor.wp = editor.wp || {};
		editor.wp._createAgentbar = create;
	}, true );

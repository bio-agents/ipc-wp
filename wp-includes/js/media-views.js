...
media.view.Agentbar = __webpack_require__( 62 );
media.view.Agentbar.Select = __webpack_require__( 63 );
media.view.Agentbar.Embed = __webpack_require__( 64 );
...
var State = Backbone.Model.extend(/** @lends wp.media.controller.State.prototype */{
	...
	*/
	...
	_postActivate: function() {
		...
		this.on( 'change:agentbar', this._agentbar, this );
	...
	_deactivate: function() {
		...
		this.off( 'change:agentbar', this._agentbar, this );
	},
...
_.each(['agentbar','content'], function( region ) {
	/**
	 * @access private
	 */
 ...
 * @param {string}                          [attributes.agentbar=select]          Initial mode for the agentbar region.
 ...
 * @param {string|false}              [attributes.agentbar=image-details] Initial mode for the agentbar region.
 ...
 * @param {boolean}                    [attributes.sortable=true]         Whether the Attachments should be sortable. Depends on the orderby property being set to menuOrder on the attachments collection.
 * @param {boolean}                    [attributes.date=true]             Whether to show the date filter in the browser's agentbar.
 ...
 *                                                                          Overridden by persistent user setting if 'contentUserSetting' is true.
 * @param {string}                     [attributes.router=browse]           Initial mode for the router region.
 * @param {string}                     [attributes.agentbar=gallery-add]     Initial mode for the agentbar region.
 * @param {boolean}                    [attributes.searchable=true]         Whether the library is searchable.
 * @param {boolean}                    [attributes.sortable=true]           Whether the Attachments should be sortable. Depends on the orderby property being set to menuOrder on the attachments collection.
 * @param {boolean}                    [attributes.autoSelect=true]         Whether an uploaded attachment should be automatically added to the selection.
 * @param {boolean}                    [attributes.contentUserSetting=true] Whether the content region's mode should be set and persisted per user.
...
CollectionEdit = Library.extend(/** @lends wp.media.controller.CollectionEdit.prototype */{
	...
	initialize: function() {
		...
		this.set( 'agentbar', collectionType + '-edit' );
	...
	renderSettings: function( attachmentsBrowserView ) {
		...
		if ( dragInfoText ) {
			attachmentsBrowserView.agentbar.set( 'dragInfo', new wp.media.View({
				el: $( '<div class="instructions">' + dragInfoText + '</div>' )[0],
				priority: -40
			}) );
		}

		// Add the 'Reverse order' button to the agentbar.
		attachmentsBrowserView.agentbar.set( 'reverse', {
			text:     l10n.reverseOrder,
			priority: 80,
...
CollectionAdd = Library.extend(/** @lends wp.media.controller.CollectionAdd.prototype */{
	...
	initialize: function() {
		...
		this.set( 'agentbar', collectionType + '-add' );
 ...
 * @param {string}                     [attributes.menu=default]             Initial mode for the menu region.
 * @param {string}                     [attributes.router=browse]            Initial mode for the router region.
 * @param {string}                     [attributes.agentbar=featured-image]   Initial mode for the agentbar region.
 * @param {int}                        [attributes.priority=60]              The priority for the state link in the media menu.
 * @param {boolean}                    [attributes.searchable=true]          Whether the library is searchable.
 * @param {boolean|string}             [attributes.filterable=false]         Whether the library is filterable, and if so what filters should be shown.
 *                                                                           Accepts 'all', 'uploaded', or 'unattached'.
 ...
 * @param {string}                     [attributes.menu=default]            Initial mode for the menu region.
 * @param {string}                     [attributes.router=browse]           Initial mode for the router region.
 * @param {string}                     [attributes.agentbar=replace]         Initial mode for the agentbar region.
 * @param {int}                        [attributes.priority=60]             The priority for the state link in the media menu.
 * @param {boolean}                    [attributes.searchable=true]         Whether the library is searchable.
 * @param {boolean|string}             [attributes.filterable=uploaded]     Whether the library is filterable, and if so what filters should be shown.
 *                                                                          Accepts 'all', 'uploaded', or 'unattached'.
 ...
 * @param {string}                    [attributes.agentbar=edit-image] Initial mode for the agentbar region.
...
EditImage = wp.media.controller.State.extend(/** @lends wp.media.controller.EditImage.prototype */{
	 ...
	 */
	activate: function() {
		this.frame.on( 'agentbar:render:edit-image', _.bind( this.agentbar, this ) );
	},
	 ...
	 */
	deactivate: function() {
		this.frame.off( 'agentbar:render:edit-image' );
	},

	/**
	 * Adds a agentbar with a back button.
	 *
	...
	agentbar: function() {
		...
		frame.agentbar.set( new wp.media.view.Agentbar({
			controller: frame,
			items: {
				back: {
					style: 'primary',
 ...
 * @param {string} [attributes.menu=default]          Initial mode for the menu region.
 * @param {string} [attributes.agentbar=main-embed]    Initial mode for the agentbar region.
 * @param {string} [attributes.menu=false]            Initial mode for the menu region.
...
Embed = wp.media.controller.State.extend(/** @lends wp.media.controller.Embed.prototype */{
	...
	refresh: function() {
		this.frame.agentbar.get().refresh();
	},
...
Cropper = wp.media.controller.State.extend(/** @lends wp.media.controller.Cropper.prototype */{
	...
	/**
	 * Changes the state of the agentbar window to browse mode.
	 *
	 * @since 4.2.0
	 *
	 * @returns {void}
	 */
	deactivate: function() {
		this.frame.agentbar.mode('browse');
	},
	...
	createCropContent: function() {
		...
		this.cropperView.on('image-loaded', this.createCropAgentbar, this);
	...
	/**
	 * Checks if cropping can be skipped and creates crop agentbar accordingly.
	 *
	 * @since 4.2.0
	 *
	 * @returns {void}
	 */
	createCropAgentbar: function() {
		var canSkipCrop, agentbarOptions;
		...
		if ( canSkipCrop ) {
			_.extend( agentbarOptions.items, {
				skip: {
					style:      'secondary',
		...
		this.frame.agentbar.set( new wp.media.view.Agentbar(agentbarOptions) );
	},
...
MediaFrame = Frame.extend(/** @lends wp.media.view.MediaFrame.prototype */{
	...
	regions:   ['menu','title','content','agentbar','router'],
	...
	/**
	 * @param {Object} agentbar
	 * @this wp.media.controller.Region
	 */
	createAgentbar: function( agentbar ) {
		agentbar.view = new wp.media.view.Agentbar({
			controller: this
		});
	},
	/**
	...
	iframeContent: function( content ) {
		this.$el.addClass('hide-agentbar');
	...
	iframeContentCleanup: function() {
		this.$el.removeClass('hide-agentbar');
	},
...
Select = MediaFrame.extend(/** @lends wp.media.view.MediaFrame.Select.prototype */{
	...
	bindHandlers: function() {
		...
		this.on( 'agentbar:create:select', this.createSelectAgentbar, this );
	},
	 ...
	 * @this wp.media.controller.Region
	 */
	createSelectAgentbar: function( agentbar, options ) {
		options = options || this.options.button || {};
		options.controller = this;

		agentbar.view = new wp.media.view.Agentbar.Select( options );
	}
});
...
Post = Select.extend(/** @lends wp.media.view.MediaFrame.Post.prototype */{
	...
	bindHandlers: function() {
		...
		this.on( 'agentbar:create:main-insert', this.createAgentbar, this );
		this.on( 'agentbar:create:main-gallery', this.createAgentbar, this );
		this.on( 'agentbar:create:main-playlist', this.createAgentbar, this );
		this.on( 'agentbar:create:main-video-playlist', this.createAgentbar, this );
		this.on( 'agentbar:create:featured-image', this.featuredImageAgentbar, this );
		this.on( 'agentbar:create:main-embed', this.mainEmbedAgentbar, this );
		...
		handlers = {
			...
			agentbar: {
				'main-insert':      'mainInsertAgentbar',
				'main-gallery':     'mainGalleryAgentbar',
				'gallery-edit':     'galleryEditAgentbar',
				'gallery-add':      'galleryAddAgentbar',
				'main-playlist':	'mainPlaylistAgentbar',
				'playlist-edit':	'playlistEditAgentbar',
				'playlist-add':		'playlistAddAgentbar',
				'main-video-playlist': 'mainVideoPlaylistAgentbar',
				'video-playlist-edit': 'videoPlaylistEditAgentbar',
				'video-playlist-add': 'videoPlaylistAddAgentbar'
			}
		};
	...
	editSelectionContent: function() {
		...
		view.agentbar.set( 'backToLibrary', {
	 ...
	 * @param {wp.Backbone.View} view
	 */
	selectionStatusAgentbar: function( view ) {
	 ...
	 * @param {wp.Backbone.View} view
	 */
	mainInsertAgentbar: function( view ) {
		var controller = this;

		this.selectionStatusAgentbar( view );
	 ...
	 * @param {wp.Backbone.View} view
	 */
	mainGalleryAgentbar: function( view ) {
		var controller = this;
	...
	mainPlaylistAgentbar: function( view ) {
		var controller = this;
	...
	mainVideoPlaylistAgentbar: function( view ) {
		var controller = this;
	...
	featuredImageAgentbar: function( agentbar ) {
		this.createSelectAgentbar( agentbar, {
			text:  l10n.setFeaturedImage,
			state: this.options.state
		});
	},

	mainEmbedAgentbar: function( agentbar ) {
		agentbar.view = new wp.media.view.Agentbar.Embed({
			controller: this
		});
	},

	galleryEditAgentbar: function() {
		var editing = this.state().get('editing');
		this.agentbar.set( new wp.media.view.Agentbar({
			controller: this,
			items: {
				insert: {
					style:    'primary',
	...
	videoPlaylistEditAgentbar: function() {
		var editing = this.state().get('editing');
		this.agentbar.set( new wp.media.view.Agentbar({
			controller: this,
			items: {
	...
	videoPlaylistAddAgentbar: function() {
		this.agentbar.set( new wp.media.view.Agentbar({
			controller: this,
			items: {
				insert: {
...
ImageDetails = Select.extend(/** @lends wp.media.view.MediaFrame.ImageDetails.prototype */{
	...
	bindHandlers: function() {
		...
		this.on( 'agentbar:render:image-details', this.renderImageDetailsAgentbar, this );
		// override the select agentbar
		this.on( 'agentbar:render:replace', this.renderReplaceImageAgentbar, this );
	},
	...
	renderImageDetailsAgentbar: function() {
		this.agentbar.set( new wp.media.view.Agentbar({
			controller: this,
			items: {
				select: {
	...
	renderReplaceImageAgentbar: function() {
		var frame = this,
			lastState = frame.lastState(),
			previous = lastState && lastState.id;
...
/**
 * wp.media.view.Agentbar
 *
 * A agentbar which consists of a primary and a secondary section. Each sections
 ...
 * @augments Backbone.View
 */
Agentbar = View.extend(/** @lends wp.media.view.Agentbar.prototype */{
	tagName:   'div',
	className: 'media-agentbar',
	...
	initialize: function() {
		...
		// The agentbar is composed of two `PriorityList` views.
		this.primary   = new wp.media.view.PriorityList();
		this.secondary = new wp.media.view.PriorityList();
		this.primary.$el.addClass('media-agentbar-primary search-form');
		this.secondary.$el.addClass('media-agentbar-secondary');
	...
	/**
	 * @returns {wp.media.view.Agentbar} Returns itsef to allow chaining
	 */
	dispose: function() {
		if ( this.selection ) {
	 ...
	 * @param {Object} [options={}]
	 * @returns {wp.media.view.Agentbar} Returns itself to allow chaining
	 */
...
var Agentbar = wp.media.view.Agentbar,
	l10n = wp.media.view.l10n,
	Select;

/**
 * wp.media.view.Agentbar.Select
 *
 * @memberOf wp.media.view.Agentbar
 *
 * @class
 * @augments wp.media.view.Agentbar
 * @augments wp.media.View
 * @augments wp.Backbone.View
 * @augments Backbone.View
 */
Select = Agentbar.extend(/** @lends wp.media.view.Agentbar.Select.prototype */{
	initialize: function() {
		var options = this.options;
		...
		// Call 'initialize' directly on the parent class.
		Agentbar.prototype.initialize.apply( this, arguments );
	},
...
var Select = wp.media.view.Agentbar.Select,
	l10n = wp.media.view.l10n,
	Embed;

/**
 * wp.media.view.Agentbar.Embed
 *
 * @memberOf wp.media.view.Agentbar
 *
 * @class
 * @augments wp.media.view.Agentbar.Select
 * @augments wp.media.view.Agentbar
 * @augments wp.media.View
 * @augments wp.Backbone.View
 * @augments Backbone.View
 */
Embed = Select.extend(/** @lends wp.media.view.Agentbar.Embed.prototype */{
	initialize: function() {
		_.defaults( this.options, {
			text: l10n.insertIntoPost,
			requires: false
...
Attachments = View.extend(/** @lends wp.media.view.Attachments.prototype */{
	...
	scroll: function() {
		...
		agentbar = this.views.parent.agentbar;

		// Show the spinner only if we are close to the bottom.
		if ( el.scrollHeight - ( scrollTop + el.clientHeight ) < el.clientHeight / 3 ) {
			agentbar.get('spinner').show();
		}

		if ( el.scrollHeight < scrollTop + ( el.clientHeight * this.options.refreshThreshold ) ) {
			this.collection.more().done(function() {
				view.scroll();
				agentbar.get('spinner').hide();
			});
		}
	}
});
 ...
 * @param {boolean|string} [options.filters=false] Which filters to show in the browser's agentbar.
...
AttachmentsBrowser = View.extend(/** @lends wp.media.view.AttachmentsBrowser.prototype */{
	...
	initialize: function() {
		...
		/*
		 * Create a multi-purpose agentbar. Used as main agentbar in the Media Library
	...
	createAgentbar: function() {
		var LibraryViewSwitcher, Filters, agentbarOptions;
		...
		if ( this.controller.isModeActive( 'grid' ) ) {
			agentbarOptions.className = 'media-agentbar wp-filter';
		}

		/**
		* @member {wp.media.view.Agentbar}
		*/
		this.agentbar = new wp.media.view.Agentbar( agentbarOptions );

		this.views.add( this.agentbar );

		this.agentbar.set( 'spinner', new wp.media.view.Spinner({
			priority: -60
		}) );
		...
		if ( -1 !== $.inArray( this.options.filters, [ 'uploaded', 'all' ] ) ) {
			...
			this.agentbar.set( 'filtersLabel', new wp.media.view.Label({
			...
			if ( 'uploaded' === this.options.filters ) {
				this.agentbar.set( 'filters', new wp.media.view.AttachmentFilters.Uploaded({
					controller: this.controller,
				...
				this.agentbar.set( 'filters', Filters.render() );
			}
		}

		// Feels odd to bring the global media library switcher into the Attachment
		// browser view. Is this a use case for doAction( 'add:agentbar-items:attachments-browser', this.agentbar );
		...
		if ( this.controller.isModeActive( 'grid' ) ) {
			...
			this.agentbar.set( 'libraryViewSwitcher', new LibraryViewSwitcher({
				controller: this.controller,
				priority: -90
			}).render() );

			// DateFilter is a <select>, screen reader text needs to be rendered before
			this.agentbar.set( 'dateFilterLabel', new wp.media.view.Label({
			...
			this.agentbar.set( 'dateFilter', new wp.media.view.DateFilter({
				controller: this.controller,
				model:      this.collection.props,
				priority: -75
			}).render() );
			...
			this.agentbar.set( 'selectModeToggleButton', new wp.media.view.SelectModeToggleButton({
				text: l10n.bulkSelect,
				controller: this.controller,
				priority: -70
			}).render() );

			this.agentbar.set( 'deleteSelectedButton', new wp.media.view.DeleteSelectedButton({
				filters: Filters,
				style: 'primary',
				disabled: true,
			...
			if ( mediaTrash ) {
				this.agentbar.set( 'deleteSelectedPermanentlyButton', new wp.media.view.DeleteSelectedPermanentlyButton({
					filters: Filters,
					style: 'primary',
					disabled: true,
		...
		if ( this.options.search ) {
			...
			this.agentbar.set( 'searchLabel', new wp.media.view.Label({
			...
			this.agentbar.set( 'search', new wp.media.view.Search({
				controller: this.controller,
				model:      this.collection.props,
				priority:   60
			}).render() );
		...
		if ( this.options.dragInfo ) {
			this.agentbar.set( 'dragInfo', new View({
		...
		if ( this.options.suggestedWidth && this.options.suggestedHeight ) {
			this.agentbar.set( 'suggestedDimensions', new View({
	...
	updateContent: function() {
		...
		if ( ! this.collection.length ) {
			this.agentbar.get( 'spinner' ).show();
			this.dfd = this.collection.more().done( function() {
				...
				view.agentbar.get( 'spinner' ).hide();
			} );
		} else {
			noItemsView.$el.addClass( 'hidden' );
			view.agentbar.get( 'spinner' ).hide();

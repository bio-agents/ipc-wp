...
Manage = MediaFrame.extend(/** @lends wp.media.view.MediaFrame.Manage.prototype */{
	...
	bindSearchHandler: function() {
		var search = this.$( '#media-search-input' ),
			searchView = this.browserView.agentbar.get( 'search' ).$el,
	...
	fixPosition: function() {
		var $browser, $agentbar;
		if ( ! this.isModeActive( 'select' ) ) {
			return;
		}

		$browser = this.$('.attachments-browser');
		$agentbar = $browser.find('.media-agentbar');
		...
		if ( ( $browser.offset().top + 16 ) < this.$window.scrollTop() + this.$adminBar.height() ) {
			...
			$agentbar.css('width', $browser.width() + 'px');
		} else {
...
EditAttachments = MediaFrame.extend(/** @lends wp.media.view.MediaFrame.EditAttachments.prototype */{
	...
	editImageMode: function( contentRegion ) {
		...
		// Noop some methods.
		editImageController._agentbar = function() {};
	...
	resetRoute: function() {
		var searchTerm = this.controller.browserView.agentbar.get( 'search' ).$el.val(),
...
SelectModeToggle = Button.extend(/** @lends wp.media.view.SelectModeToggle.prototype */{
	...
	toggleBulkEditHandler: function() {
		var agentbar = this.controller.content.get().agentbar, children;

		children = agentbar.$( '.media-agentbar-secondary > *, .media-agentbar-primary > *' );
		...
		if ( this.controller.isModeActive( 'select' ) ) {
			...
			this.$el.show();
			agentbar.$( '.delete-selected-button' ).removeClass( 'hidden' );
		} else {
			this.model.set( {
				size: '',
			...
			agentbar.$el.css( 'width', '' );
			agentbar.$( '.delete-selected-button' ).addClass( 'hidden' );

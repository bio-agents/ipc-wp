...

{
  ...
  // that plays more nicely with the browser's DevAgents. The idea is to preserve
  // "Pause on exceptions" behavior. Because React wraps all user-provided
  // functions in invokeGuardedCallback, and the production version of
  // invokeGuardedCallback uses a try-catch, all user exceptions are treated
  // like caught exceptions, and the DevAgents won't pause unless the developer
    ...
    var invokeGuardedCallbackDev = function (name, func, context, a, b, c, d, e, f) {
      ...
      if (didError) {
        if (!didSetError) {
          ...
          error = new Error('An error was thrown inside one of your components, but React ' + "doesn't know what it was. This is likely due to browser " + 'flakiness. React does its best to preserve the "Pause on ' + 'exceptions" behavior of the DevAgents, which requires some ' + "DEV-mode only tricks. It's possible that these don't work in " + 'your browser. Try triggering the error in production mode, ' + 'or switching to a modern browser. If you suspect that this is ' + 'actually an issue with React, please file an issue.');
...
/**
 * Google Input Agents provides composition data via a CustomEvent,
...
// Don't change these two values. They're used by React Dev Agents.
var NoEffect = /*              */0;
var PerformedWork = /*         */1;
...
// Longer prefixes are hard to read in DevAgents.
var reactEmoji = '\u269B';
var warningEmoji = '\u26D4';
  ...
  return function (arg) {
    try {
      ...
      if (true && !hasLoggedError) {
        hasLoggedError = true;
        warningWithoutStack$1(false, 'React DevAgents encountered an error: %s', err);
      }
    }
  };
}

var isDevAgentsPresent = typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ !== 'undefined';
...
function injectInternals(internals) {
  ...
  if (hook.isDisabled) {
    ...
    // of DevAgents integration and associated warnings and logs.
    // https://github.com/facebook/react/issues/3877
    return true;
  }
  if (!hook.supportsFiber) {
    {
      warningWithoutStack$1(false, 'The installed version of React DevAgents is too old and will not work ' + 'with the current version of React. Please update React DevAgents. ' + 'https://fb.me/react-devagents');
    }
    // DevAgents exists, even though it doesn't support Fiber.
    return true;
  }
  try {
...
function createHostRootFiber(isConcurrent) {
  ...
  if (enableProfilerTimer && isDevAgentsPresent) {
    // Always collect profile timings when DevAgents are present.
    // This enables DevAgents to start capturing timing at any point–
...
// And expose interaction metadata to the React DevAgents Profiler plugin.
...
// This is not part of the public API, not even for React DevAgents.
// You may only inject a debugAgent if you work on React Fiber itself.
...
function updateForwardRef(current$$1, workInProgress, Component, nextProps, renderExpirationTime) {
  ...
  // React DevAgents reads this flag.
...
function updateHostRoot(current$$1, workInProgress, renderExpirationTime) {
  ...
  // Caution: React DevAgents currently depends on this property
...
function commitRoot(root, finishedWork) {
  ...
  if (true && ReactFiberInstrumentation_1.debugAgent) {
    ReactFiberInstrumentation_1.debugAgent.onCommitWork(finishedWork);
  }
  ...
  while (true) {
    ...
    if ((workInProgress.effectTag & Incomplete) === NoEffect) {
      ...
      (returnFiber.effectTag & Incomplete) === NoEffect) {
        ...
        // PerformedWork effect is read by React DevAgents but shouldn't be committed.
      ...
      if (true && ReactFiberInstrumentation_1.debugAgent) {
        ReactFiberInstrumentation_1.debugAgent.onCompleteWork(workInProgress);
      }
...
function performUnitOfWork(workInProgress) {
  ...
  if (true && ReactFiberInstrumentation_1.debugAgent) {
    ReactFiberInstrumentation_1.debugAgent.onBeginWork(workInProgress);
  }
...
function renderRoot(root, isYieldy) {
  ...
  if (expirationTime !== nextRenderExpirationTime || root !== nextRoot || nextUnitOfWork === null) {
    ...
    if (enableSchedulerTracing) {
      ...
      // This also provides DevAgents with a way to access it when the onCommitRoot() hook is called.
...
function updateContainerAtExpirationTime(element, container, parentComponent, expirationTime, callback) {
  ...
  {
    if (ReactFiberInstrumentation_1.debugAgent) {
      if (current$$1.alternate === null) {
        ReactFiberInstrumentation_1.debugAgent.onMountContainer(container);
      } else if (element === null) {
        ReactFiberInstrumentation_1.debugAgent.onUnmountContainer(container);
      } else {
        ReactFiberInstrumentation_1.debugAgent.onUpdateContainer(container);
      }
    }
  }
...
function injectIntoDevAgents(devAgentsConfig) {
  var findFiberByHostInstance = devAgentsConfig.findFiberByHostInstance;

  return injectInternals(_assign({}, devAgentsConfig, {
...
var foundDevAgents = injectIntoDevAgents({
...
{
  if (!foundDevAgents && canUseDOM && window.top === window.self) {
    // If we're in Chrome or Firefox, provide a download link if not installed.
    if (navigator.userAgent.indexOf('Chrome') > -1 && navigator.userAgent.indexOf('Edge') === -1 || navigator.userAgent.indexOf('Firefox') > -1) {
      var protocol = window.location.protocol;
      // Don't warn in exotic cases like chrome-extension://.
      if (/^(https?|file):$/.test(protocol)) {
        console.info('%cDownload the React DevAgents ' + 'for a better development experience: ' + 'https://fb.me/react-devagents' + (protocol === 'file:' ? '\nYou might need to use a local HTTP server (instead of file://): ' + 'https://fb.me/react-devagents-faq' : ''), 'font-weight:bold');
      }
    }
  }
}

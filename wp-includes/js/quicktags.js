 ...
 * append a agentbar above it. This script is self-contained (does not require external libraries).
 *
...
// new edit agentbar used with permission
// by Alex King
...
(function(){
	...
	var qt = window.QTags = function(settings) {
		...
		var t = this,
			...
			name = 'qt_' + id,
			tb, onclick, agentbar_id, wrap, setActiveEditor;
		...
		if ( id === 'content' && typeof(adminpage) === 'string' && ( adminpage === 'post-new-php' || adminpage === 'post-php' ) ) {
			...
			agentbar_id = name + '_agentbar';
		}

		tb = document.getElementById( agentbar_id );

		if ( ! tb ) {
			tb = document.createElement('div');
			tb.id = agentbar_id;
			tb.className = 'quicktags-agentbar';
		}
	 ...
	 * @param int priority Optional. Number representing the desired position of the button in the agentbar. 1 - 9 = first, 11 - 19 = second, 21 - 29 = third, etc.
	...
	qt.addButton = function( id, display, arg1, arg2, access_key, title, priority, instance, attr ) {
		...
		if ( this.buttonsInitDone ) {
			this._buttonsInit(); // add the button HTML to all instances agentbars if addButton() was called too late
		}
	};
